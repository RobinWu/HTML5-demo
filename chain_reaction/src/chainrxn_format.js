...
if(this.yposition< = this.gameLevel.ballSize||this.yposition> = this.gameLevel.ymax-this.gameLevel.ballSize){
  ...
  startExpansion : function(){
    ...
    this.gameLevel.chainrxn.totalScoreEl.set("text",
    this.gameLevel.score+this.gameLevel.chainrxn.score+" total points");
    this.downspeed = 0;
    this.rightspeed = 0;
    this.expanding = 1;
...
var coordinates = this.gameLevel.chainrxn.ballField.getCoordinates();
    ...
    (function(){
      pointBox.destroy();
    }).delay(this.gameLevel.expandedLifeLength,
    this);
  },
  expand : function(){
...
if(this.yposition>max_y){
  ...
  place : function(e){
    ...
    this.checkBounds();
    var coordinates = this.gameLevel.chainrxn.ballField.getCoordinates();
    new Ball(this.gameLevel,
    this.gameLevel.numBalls,
    this.xposition-coordinates.left,
    this.yposition-coordinates.top);
...
var GameLevel = new Class({
  ...
  pointPopupWidth : 50,
  canvas : false,
  initialize : function(ballDetails,
  chainrxn){
    this.chainrxn = chainrxn;
    this.chainrxn.ballField.setStyle("background-color",
    "#333");
    if(this.chainrxn.ballField.getContext){
      this.canvas = this.chainrxn.ballField.getContext("2d");
    }else{
      return alert("Your browser does not appear to support canvas,
       and our attempts to emulate it have failed.");
    }
...
this.movingBalls.each(function(ball,
      index){
        ball.maintain();
      }.bind(this))
...
$$("body")[0].addEvent("click",
    function(e){
      if(e.target.id! = "notifierButton"){
        starterBall.place(e);
        starterBall = null;
        this.chainrxn.levelNumberEl.set("text",
        "Level "+(this.chainrxn.levelNumber+1));
      }
    }.bind(this))
  },
  showWinBG : function(){
