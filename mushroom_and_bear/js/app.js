...
function addEventHandlers() {
    ...
    $("#container").mousemove(function(e){ mushroom.x = e.pageX - (mushroom.image.width/2); });

    //Add event handler for start button
    $("#BtnImgStart").click(function (){ toggleGameplay(); });

    //Add event handler for show/hide bounds button  
    $("#showHideBounds").click(function (){ showBounds = !showBounds; gameLoop(); });
}
function startGame() {
    score=0;
    lives=3;
...
function startGameTimer() {
    ...
    if(gameRunning) window.setTimeout(timer, 15);
}
...
function drawBaddy() {
    ...
    if(checkIntersect(baddy, animal, 5)) {
        ...
        if(baddyStatus != BADDY_HIT) horizontalSpeed = -horizontalSpeed;
    ...
    if(baddyStatus == BADDY_OFF) baddy.x = 500;

    //If baddy (owl) is flying, then move it from right to left
    if(baddyStatus == BADDY_FLYING) baddy.x -=2;
...
function drawBonus() {
    if(bonusActive) ctx.drawImage(bonusImages[currentBonusImage], 200, 150);
}
//Draw the mushroom (depending on its current status)
...
function changeBaddyImage() {
    ...
    if(currentBaddyImage == 2) currentBaddyImage = 0;
    if(baddyStatus != BADDY_OFF) setTimeout(changeBaddyImage, 100);
}
function showBonus() {
    if(!bonusActive) {
        bonusStarted = new Date().getTime();
...
function changeBonusImage() {
    currentBonusImage++;
    if(currentBonusImage == 3) currentBonusImage = 0;
